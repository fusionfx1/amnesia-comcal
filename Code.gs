/**
 * Google Apps Script for AMNESIA Staff Commission 2025
...
function loadCommissionData(month) {
  try {
    ...
    const result = filteredData.map(row => ({
      ...
      netCommission: parseFloat(row[8]) || 0,
      netOT: parseFloat(row[9]) || 0,
      finalAmount: parseFloat(row[10]) || 0
    }));
...
function formatCurrencyColumns(sheet, startRow, numRows) {
  try {
    // Currency columns: Total Sales, Overtime, Sales Ex-VAT, Commission, Net Commission, Net OT, Final Amount
    const currencyColumns = [4, 6, 7, 8, 9, 10, 11];
...
function createDashboard() {
  ...
  if (!dashboardSheet) {
    ...
    dashboardSheet.getRange('A3').setValue('Monthly Summary');
...
function createMonthlySummaryChart(dashboardSheet) {
  ...
  if (dataSheet.getLastRow() > 1) {
    ...
    const chartBuilder = dashboardSheet.newChart()
      ...
      .addRange(dataSheet.getRange('K2:K'))
      .setPosition(4, 1, 0, 0)
      .setOption('title', 'Monthly Commission Totals')
...
function onOpen() {
  ...
  ui.createMenu('Commission Tools')
    ...
    .addItem('Test Save Data', 'testSaveData')
    .addItem('Test Load Data', 'testLoadData')
    .addToUi();
}
...
function getEmployeeSummary() {
  try {
    ...
    const summary = employees.map(employee => {
      ...
      const totalCommission = employeeData.reduce((sum, row) => sum + (parseFloat(row[10]) || 0), 0);
